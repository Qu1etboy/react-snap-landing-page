...
const Navbar = () => {
  ...
  const [isOpenFeature, setOpenFeature] = useState(false);
  const [isOpenCompany, setOpenCompany] = useState(false);

  const Feature = ({ clickable = false, children }) => (
    <div className="relative">
      <div
        className="peer flex items-center gap-3 cursor-pointer text-[#696969] hover:text-black"
        onClick={() => {
          if (clickable) {
            setOpenFeature(!isOpenFeature);
          }
        }}
      >
        <a>Features </a>
        {isOpenFeature && clickable ? (
  ...
  const FeatureDropdown = () => (
    <>
      <div className="flex gap-3 items-center">
        ...
        <a className="text-[#696969] cursor-pointer hover:text-black">
          Todo List
        </a>
      </div>
      <div className="flex gap-3 items-center">
  ...
  const Company = ({ clickable = false, children }) => (
    <div className="relative">
      <div
        className="peer flex items-center gap-3 cursor-pointer text-[#696969] hover:text-black"
        onClick={() => {
  ...
  const CompanyDropDown = () => (
    <>
      <a className="text-[#696969] cursor-pointer hover:text-black">History</a>
      <a className="text-[#696969] cursor-pointer hover:text-black">Our Team</a>
      <a className="text-[#696969] cursor-pointer hover:text-black">Blog</a>
    </>
  );

  const Careers = () => (
    <a className="cursor-pointer text-[#696969] hover:text-black">Careers</a>
  );

  const About = () => (
    <a className="cursor-pointer text-[#696969] hover:text-black">About</a>
  );

  const LoginButton = () => (
    <button className="text-[#696969] hover:text-black">Login</button>
  );

  const RegisterButton = () => (
    <button className="border border-[#696969] rounded-xl px-3 py-2 text-[#696969] hover:text-black hover:border-black">
      Register
    </button>
  );
  return (
    <>
      <div className="w-full flex justify-between px-10 pt-10 lg:pt-5 pb-5">
        <div className="flex gap-10 justify-center items-center lg:items-center">
          ...
          <div className="hidden lg:flex gap-10 justify-center items-center">
            <Feature>
              <div
                className={`peer-hover:flex hover:flex hidden absolute right-0 rounded-lg px-5 py-5 flex-col gap-3 bg-white drop-shadow-xl`}
              >
                <FeatureDropdown />
              </div>
            </Feature>
            <Company>
              <div
                className={`peer-hover:flex hover:flex hidden absolute w-[120px] rounded-lg px-5 py-5 flex-col gap-3 bg-white drop-shadow-xl`}
              >
                <CompanyDropDown />
              </div>
            </Company>
        ...
        <div
          className="flex items-start lg:hidden cursor-pointer"
      ...
      <div
        ...
        } h-screen w-full bg-black bg-opacity-50`}
      >
        <div
          id="drawer-navigation"
          className={`fixed right-0 z-40 h-screen p-4 overflow-y-auto bg-white w-80 ${
            isOpenMenu ? "menu-in" : "menu-out"
          }`}
          tabIndex="-1"
        ...
        >
          ...
          <div className="py-10 px-5 flex flex-col gap-5">
            <Feature clickable={true}>
              <div
                className={`${
                  isOpenFeature ? "flex" : "hidden"
                } rounded-lg px-5 py-5 flex-col gap-3 bg-white`}
              >
                <FeatureDropdown />
              </div>
            </Feature>
            <Company clickable={true}>
              <div
                className={`${
                  isOpenCompany ? "flex" : "hidden"
                } rounded-lg px-5 py-5 flex-col gap-3 bg-white`}
              >
                <CompanyDropDown />
              </div>
